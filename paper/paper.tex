...
\begin{table}[h]
\caption{Best fitness by topology at different generations (Trap-5,
  15 seeds). Bold = best at that generation.}
\label{tab:convergence}
\centering
\small
\begin{tabular}{rcccccc}
\toprule
\textbf{Gen} & \textbf{$K_4{-}e{+}$p} & \textbf{Trap-5 ev.} & \textbf{HIFF ev.} & \textbf{FC} & \textbf{Ring} & \textbf{None} \\
\midrule
 25  & \textbf{0.849} & 0.844 & 0.829 & 0.848 & 0.837 & 0.812 \\
 50  & 0.881 & 0.892 & 0.872 & \textbf{0.895} & 0.883 & 0.847 \\
100  & 0.899 & \textbf{0.909} & 0.899 & 0.907 & 0.908 & 0.856 \\
200  & 0.900 & \textbf{0.920} & 0.911 & 0.907 & 0.913 & 0.853 \\
300  & 0.904 & 0.916 & 0.913 & 0.907 & \textbf{0.919} & 0.851 \\
500  & 0.904 & 0.919 & 0.920 & 0.908 & \textbf{0.924} & 0.855 \\
\bottomrule
\end{tabular}
\end{table}

On Trap-5: the $K_4{-}e$ + pendant graph leads at generation~25
(fast exploration), but \textbf{ring overtakes all topologies by
generation~300} and reaches 0.924 at generation~500. FC peaks early
and stagnates. The graph we evolved for Trap-5 (optimized at the
200-generation horizon) performs well at generation~200 but is not
the long-run winner.

On HIFF: the pattern is similar but more extreme. The $K_4{-}e$ +
pendant leads through generation~100, but the sparser HIFF-evolved
graph ($\lambda_2 = 0.52$) overtakes it by generation~300 and reaches
0.815 at generation~500.

\subsection{Different Graphs Peak at Different Times}

\begin{table}[h]
\caption{Generation at which each topology reaches peak fitness
  (Trap-5 / HIFF).}
\label{tab:peaks}
\centering
\begin{tabular}{lrrrr}
...
\midrule
$K_4{-}e$ + pendant  & 229 & 0.907 & 425 & 0.792 \\
FC                   & 410 & 0.911 & 348 & 0.762 \\
Ring                 & 421 & 0.925 & 476 & 0.799 \\
HIFF evolved         & 453 & 0.923 & 482 & 0.818 \\
None                 & 383 & 0.863 & 428 & 0.709 \\
\bottomrule
\end{tabular}
\end{table}

On HIFF, FC peaks earliest (generation~348) and then degrades---exactly
the over-mixing effect where aggressive migration destroys hierarchical
building blocks. Sparser topologies peak later but reach higher
fitness. The $K_4{-}e$ + pendant graph is a \emph{fast starter}: it
provides the best early-generation performance but is not the
asymptotic winner.
...
asymptotic fitness. A different inner horizon would produce a different
``optimal'' topology:

\begin{itemize}
\item \textbf{Short horizon ($< 100$ gens):} Dense topologies win
  (fast assembly). The $K_4{-}e$ + pendant graph excels here.
\item \textbf{Medium horizon (100--300 gens):} The evolved
  domain-specific graphs win. This is where our experiments operated.
\item \textbf{Long horizon ($> 300$ gens):} Sparser topologies win
  (sustained diversity prevents stagnation). Ring overtakes everything
  on Trap-5.
\end{itemize}

The evaluation horizon is not a nuisance parameter---it is a
\emph{design choice} that determines which topology is optimal.
``Best topology for deceptive landscapes'' is meaningless without
specifying ``best at what generation.''
