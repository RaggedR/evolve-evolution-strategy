...
fully connected wins; on HIFF ring/random win; on MMDP and overlapping
traps intermediate topologies win. The diversity ordering is a
structural invariant; the fitness ordering is not.

We then evolve the migration graph itself using an outer genetic
algorithm. Across three of five domains, the outer GA independently
converges to the \emph{same graph}: $K_4 - e$ plus a pendant vertex
(6 edges, $\lambda_2 = 0.83$, degree sequence $\{1,2,3,3,3\}$).
This graph---a dense triangle core with a semi-isolated diversity
reservoir---outperforms all canonical topologies and emerges as a
universal architecture for deceptive landscapes.
\end{abstract}
...
\item \textbf{Fitness inversion.} On deceptive landscapes, the fitness
  ordering inverts: more connectivity helps rather than hurts, because
  building block assembly dominates over diversity preservation.
  Five deceptive domains produce four distinct fitness orderings while
  the diversity ordering remains canonical (Section~\ref{sec:inversion}).

\item \textbf{Evolved migration graphs.} An outer GA evolving the
  migration topology discovers that moderate connectivity with
  structural asymmetry outperforms all canonical topologies
  (Section~\ref{sec:evolving}).

\item \textbf{A universal topology.} The outer GA independently
  converges to the same graph---$K_4 - e$ plus a pendant vertex---on
  three of five domains. This is the first identification of a
  universal migration architecture for deceptive landscapes
...
200 generations for trap domains, 300 for HIFF.
...
\textbf{TODO:} Discussion of why evolved graph sometimes loses on
30-seed comparison (noisy inner fitness with 5 seeds).
...
\begin{itemize}
\item Inner GA fitness is noisy (5 seeds per evaluation). The evolved
  graph sometimes loses to canonical topologies on 30-seed comparison
  (MMDP, overlapping traps).
\item Only $n = 5$ and $n = 8$ islands tested. The isomorphism result
  is specific to $n = 5$.
\item Binary adjacency genomes cannot express directed or weighted
  edges, which might help on epistatic domains.
\end{itemize}
...
diversity reservoir. This is, to our knowledge, the first
identification of a universal migration architecture and the first
application of meta-evolution to the island-model topology.
