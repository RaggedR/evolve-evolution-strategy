...
At $n = 8$ islands (28-bit graph genome, $2^{28} \approx 268$M
possible graphs), the evolved graph beats FC by +0.009 on Trap-5
(vs +0.005 at $n = 5$). The advantage over canonical topologies
grows with island count. Evolved graphs at $n = 8$ maintain
$\sim$50\% edge density and heterogeneous degree distribution, but
the two graphs evolved (for Trap-5 and Trap-7) are not isomorphic---the
larger search space may require more outer GA generations to converge.
...
\textbf{HIFF ($n=5$):} The $K_4{-}e$ + pendant leads through
generation~100 (fast exploration). The HIFF-evolved graph
($\lambda_2 = 0.52$) overtakes at generation~300 and leads through
generation~1000. At generation~1500, star overtakes---reaching 0.844
at generation~2000. However, this is an artifact of the $n=5$ scale
(see below).

Notably, none (isolated islands) shows severe degradation on HIFF:
peak 0.818 then falling to 0.763, a loss of 0.055. Diversity without
any exchange eventually becomes genetic drift.

\subsection{The Ring/Star Spectral Crossover}

The ACT paper~\cite{langer2026} predicts that ring and star swap
their diversity ordering at $n = 7$: ring $\lambda_2 \approx
4\pi^2/n^2$ decreases with $n$, while star $\lambda_2 = 1$ is
constant. At $n \geq 7$, ring has lower $\lambda_2$ (slower mixing,
more diversity). We test whether this spectral crossover affects
\emph{fitness} ordering on HIFF at 2000 generations.

\begin{table}[h]
\caption{HIFF long-run fitness: ring vs star across island counts.
  Bold = winner at generation 2000.}
\label{tab:crossover}
\centering
\begin{tabular}{rrrcccc}
\toprule
$n$ & Ring $\lambda_2$ & Star $\lambda_2$ & Ring@2000 & Star@2000 & Ring degrad. & Star degrad. \\
\midrule
5 & 1.382 & 1.000 & 0.808 & \textbf{0.844} & $-0.004$ & $-0.007$ \\
7 & 0.753 & 1.000 & \textbf{0.862} & 0.858 & $\mathbf{0.000}$ & $-0.008$ \\
8 & 0.586 & 1.000 & \textbf{0.871} & 0.835 & $\mathbf{0.000}$ & $-0.013$ \\
\bottomrule
\end{tabular}
\end{table}

The crossover is confirmed: \textbf{at $n \geq 7$, ring overtakes star
on HIFF fitness}, exactly as the spectral theory predicts. Star's
advantage at $n = 5$ is an artifact of ring having higher $\lambda_2$
than star at that scale.

Two further observations:
\begin{itemize}
\item \textbf{Ring shows zero degradation} at $n = 7$ and $n = 8$,
  while star degrades increasingly ($-0.008$ at $n=7$, $-0.013$ at
  $n=8$). Ring is the only topology that never overfits.
\item \textbf{None degrades catastrophically} at $n=8$: peak 0.862
  then crash to 0.779 ($-0.083$). Isolation without exchange becomes
  drift at scale.
\end{itemize}

This is the first experimental confirmation that the spectral
prediction from~\cite{langer2026} extends to \emph{fitness} ordering
on deceptive landscapes, not just diversity ordering.
...
\subsection{Mechanistic Decomposition: $\beta_1$ and $\lambda_2$ as Independent Invariants}
\label{sec:bridge}

The time-dependent results above describe \emph{what} changes over generations;
the following controlled experiment identifies \emph{why}.

We constructed two iso-spectral graph families for 8-island topologies:

\begin{itemize}
\item \textbf{Ring family:} $C_8$ plus 0--3 antipodal chords.
  $\lambda_2$ held constant at $2 - \sqrt{2} \approx 0.586$;
  $\beta_1$ varies from 1 to 4.
\item \textbf{Star family:} $S_8$ plus 0--3 leaf--leaf edges.
  $\lambda_2 = 1.0$ (constant); $\beta_1$ varies from 0 to 3.
\end{itemize}

Within each family, any observed effect on diversity is attributable to
$\beta_1$ alone---$\lambda_2$ is held fixed. Across families, $\beta_1$
is held fixed while $\lambda_2$ varies, isolating the spectral effect.

\textbf{Results (320 runs, NK landscape $K = 4$):}

\begin{table}[h]
\caption{Effect sizes ($\eta^2$) for $\beta_1$ and $\lambda_2$ on diversity at three
  generations. Within-family variation isolates $\beta_1$; cross-family
  variation isolates $\lambda_2$.}
\label{tab:bridge}
\centering
\begin{tabular}{lrrr}
\toprule
\textbf{Invariant} & \textbf{Gen 100} & \textbf{Gen 200} & \textbf{Gen 500} \\
\midrule
$\beta_1$ (cycle rank) & 0.191 & 0.162 & $\approx 0$ (n.s.) \\
$\lambda_2$ (spectral gap) & --- & 0.260 & 0.260 \\
\bottomrule
\end{tabular}
\end{table}

Cycle rank $\beta_1$ produces a \emph{transient} effect: strong at
generation~100 ($\eta^2 = 0.191$), declining by generation~200, and
statistically indistinguishable from zero by generation~500. Algebraic
connectivity $\lambda_2$ produces a \emph{persistent} effect: stronger
than $\beta_1$'s peak at generation~200 ($\eta^2 = 0.26$) and
maintained at generation~500.

The two invariants are complementary, not redundant: $\beta_1$ controls
transient recirculation of building blocks during early search;
$\lambda_2$ controls long-run mixing rate and equilibrium diversity
level. This provides a mechanistic explanation for the time-dependent
fitness results: the early-generation advantage of $K_4 - e$ + pendant
is driven by its triangle-core cycle structure (high $\beta_1$) creating
a building-block assembly window; its equilibrium behaviour is
determined by $\lambda_2 = 0.83$, which sits between ring ($\lambda_2
\to 0$ at large $n$) and FC ($\lambda_2 = n/(n-1)$).
...
\item \textbf{Long horizon ($> 1000$ gens):} Ring dominates Trap-5
  (sustained diversity, zero degradation). On HIFF, star leads at
  $n=5$ but ring overtakes at $n \geq 7$---consistent with the
  spectral crossover prediction (Section~\ref{sec:time}).
\end{itemize}
...
\item \textbf{Honest landscapes.} On four honest domains (knapsack,
  maze, graph coloring, sorting network), the fitness spread between
  topologies is $< 0.004$---topology does not meaningfully affect
  fitness. The evolved graphs show no convergent structure (no
  isomorphisms, random degree sequences). The $K_4 - e$ + pendant
  architecture is specific to deceptive landscapes.
\end{itemize}
...
Three levels of structure govern island-model dynamics:
\begin{enumerate}
\item $\lambda_2$ determines diversity ordering (a structural invariant,
  domain-independent).
\item Graph \emph{shape} determines short-horizon fitness (the
  convergent $K_4 - e$ + pendant for deceptive landscapes; no
  convergent structure for honest landscapes).
\item The evaluation horizon determines which topology wins long-run
  (ring dominates asymptotically on traps; the ring/star crossover at
  $n = 7$ on HIFF confirms the spectral prediction extends to fitness).
\end{enumerate}

Iso-spectral bridge experiments (320 runs) decompose the time-dependence
mechanistically: $\beta_1$ (cycle rank) governs a transient diversity
window peaking at generation~100 ($\eta^2 = 0.191$) and collapsing by
generation~500; $\lambda_2$ governs equilibrium diversity with
$\eta^2 = 0.26$ at generation~200, persisting to generation~500.
The two invariants operate on independent timescales.
On honest landscapes, none of these levels differentiates topologies---the
fitness spread is $< 0.004$. The entire phenomenon is specific to
deception.
